...
const TagSelect: React.FC<TagSelectProps> & { Option: typeof TagSelectOption } = (props) => {
    ...
    const cls = classNames(styles['tag-select'], className, {
        [styles['has-expand-tag']]: expandable,
        [styles['expanded']]: expand
    })

    return <div className={cls} style={style}>
        {hideCheckAll ? null : (
            <Tag.CheckableTag checked={checkedAll} key="tag-select-__all__" onChange={onSelectAll}>
                {selectAllText}
            </Tag.CheckableTag>
        )}
        {children &&
